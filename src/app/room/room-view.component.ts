...
//   account: any = null; // TODO: set this from your auth/account service in ngOnInit
...
//       next: r => this.receivedItems = r?.items || r || [],
//       error: e => this.alert.error(e)
//     });
...
//       const stockroomType = this.scanned.itemType || 'apparel';
...
//    * Examples:
//    *    Content-Disposition: attachment; filename="qr-apparel-123.png"
//    *    Content-Disposition: attachment; filename*=UTF-8''qr-apparel-123.png
//    */
...
//   /**
//    * Called from template when user clicks QR button for a batch/inventory object
//    */
...
//   /**
//    * Called from template when user clicks QR button for a unit
//    */
//   onDownloadUnitQr(unit: any) {
//     const stockroomType = (this.room && this.room.stockroomType) ? this.room.stockroomType : 'apparel';
//     const unitId = unit.apparelId || unit.adminSupplyId || unit.id;
...
//   selectInventoryForRelease(inv: any) {
//     this.openReleaseModal(inv.id || inv.apparelInventoryId || inv.inventoryId);
//   }

//   onRegister() {
//     if (this.registerForm.invalid) return;
//     this.roomService.registerItem(this.roomId, this.registerForm.value.itemId).pipe(first()).subscribe({
//       next: () => { this.alert.success('Item registered'); this.loadAll(); this.registerForm.reset(); },
//       error: e => this.alert.error(e)
//     });
//   }
...
//     const { itemQrCode, newStatus } = this.statusForm.value;
//     this.roomService.updateItemStatus(this.roomId, itemQrCode, newStatus).pipe(first()).subscribe({
//       next: () => { this.alert.success('Item status updated'); this.loadAll(); this.statusForm.reset(); },
//       error: e => this.alert.error(e)
//     });
//   }
// }
    ...
    this.roomService.getRoomById(this.roomId).pipe(first()).subscribe({
      next: r => {
        ...
        if (!st || st === 'apparel') {
          ...
          // released batches for supply may not exist server-side — attempt if present
          this.roomService.getReleasedBatches(this.roomId).pipe(first()).subscribe({
            next: r => this.releasedBatches = r?.batches || r || [],
            error: () => {}
          });
