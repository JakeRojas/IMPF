...
export class ItReleaseComponent implements OnInit {
    roomId = NaN;
    inventory: any[] = [];
    ...
    ngOnInit(): void {
        ...
        this.releaseForm = this.fb.group({
            itInventoryId: [null, Validators.required],
    ...
    submit() {
        ...
        if (payload.itInventoryId != null) payload.itInventoryId = Number(payload.itInventoryId);

        if (payload.remarks && !payload.notes) payload.notes = payload.remarks;

        this.roomService.releaseIt(this.roomId, payload).pipe(first()).subscribe({
            next: () => {
                this.alert.success('IT item released successfully');
