...
//   ngOnInit() {
//     //this.stockRequestId = +this.router.snapshot.params['stockRequestId'];
//     this.load();
//   }

//   load() {
//     this.loading = true;
//     this.sr.get(this.stockRequestId).pipe(first()).subscribe({
//       next: (res: any) => { this.request = (res?.data || res); this.loading = false; },
//       error: e => { this.alert.error(e); this.loading = false; }
//     });
//   }

//   approve() { if (!confirm('Approve?')) return; this.sr.approve(this.stockRequestId).pipe(first()).subscribe(() => { this.alert.success('Approved'); this.load(); }, e => this.alert.error(e)); }
//   disapprove() { const r = prompt('Reason?'); this.sr.disapprove(this.stockRequestId).pipe(first()).subscribe(() => { this.alert.success('Disapproved'); this.load(); }, e => this.alert.error(e)); }
//   fulfill() { if (!confirm('Fulfill?')) return; this.sr.fulfill(this.stockRequestId).pipe(first()).subscribe(() => { this.alert.success('Fulfilled'); this.load(); }, e => this.alert.error(e)); }
// }


// src/app/requests/stock-request/stock-request-view.component.ts
// src/app/requests/stock-request/stock-request-view.component.ts
import { Component, OnInit } from '@angular/core';
import { ActivatedRoute, Router } from '@angular/router';
import { StockRequestService } from '@app/_services/stock-request.service';
...
export class StockRequestViewComponent implements OnInit {
  ...
  //   this.sr.fulfill(this.id).pipe(first()).subscribe({
  //     next: (res) => {
  //       this.alert.success('Fulfilled');
  //       this.load();
  //     },
  //     error: err => {
  //       console.error('Fulfill error (full):', err);
  //       // Prefer server-sent message if any
  //       const msg = err?.error?.message ?? err?.message ?? 'Forbidden';
